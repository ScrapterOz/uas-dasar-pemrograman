...
class CatatanPengeluaran:
    ...
    def _tampilkan_tabel(self, header, data=None):
        lebar_kolom = 20

        # Menampilkan header
        for kolom in header:
            print(f"{str(kolom).center(lebar_kolom)}|", end="")
        print()

        # Menampilkan garis pemisah
        print("-" * (lebar_kolom * len(header) + len(header) - 1))

        # Menampilkan data jika ada
        if data is not None:
            for transaksi in data:
                for kolom in transaksi:
                    if isinstance(transaksi[kolom], str):
                        print(f"{transaksi[kolom].center(lebar_kolom)}|", end="")
                    else:
                        print(f"{str(transaksi[kolom]).center(lebar_kolom)}|", end="")
                print()
